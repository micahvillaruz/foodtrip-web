<!-- list.js min js -->
<script src="<?php echo base_url('public/assets') ?>/libs/list.js/list.min.js"></script>

<!--list pagination js-->
<script src="<?php echo base_url('public/assets') ?>/libs/list.pagination.js/list.pagination.min.js"></script>

<!-- ecommerce-order init js -->
<script src="<?php echo base_url('public/assets') ?>/js/pages/ecommerce-order.init.js"></script>

<!-- Sweet Alerts js -->
<script src="<?php echo base_url('public/assets') ?>/libs/sweetalert2/sweetalert2.min.js"></script>

<!-- App js -->
<script src="<?php echo base_url('public/assets') ?>/js/app.js"></script>