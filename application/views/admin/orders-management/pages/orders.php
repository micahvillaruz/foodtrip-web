<div class="row">
    <div class="col-lg-12">
        <div class="card" id="orderList">
            <div class="card-header border-bottom-dashed">
                <div class="row g-4 align-items-center">
                    <div class="col-sm">
                        <div>
                            <h5 class="card-title mb-0">Order History</h5>
                        </div>
                    </div>
                    <div class="col-sm-auto">
                        <div id="export-buttons">
                        </div>
                    </div>
                </div>
            </div>
            <div class="card-body border border-dashed border-end-0 border-start-0">
                <table id="buttons-datatables" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                    <thead>
                        <tr>
                            <th>No.</th>
                            <th>Customer</th>
                            <th>Restaurant</th>
                            <th>Order Total</th>
                            <th>Date Ordered</th>
                            <th>Order Status</th>
                            <th>Date Released</th>
                            <th>Date Rejected</th>
                            <th>Date Delivered</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>48140400825</td>
                            <td>Kalix Jace Martinez</td>
                            <td>Jollibee</td>
                            <td>₱550.00</td>
                            <td>09 Jul,2022 2:01 PM</td>
                            <td><span class="badge badge-soft-success text-uppercase">Delivered</span></td>
                            <td>09 Jul,2022 2:20 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 2:55 PM</td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>61097516414</td>
                            <td>Louisse Natasha Valeria</td>
                            <td>McDonald's</td>
                            <td>₱249.00</td>
                            <td>09 Jul,2022 2:03 PM</td>
                            <td><span class="badge badge-soft-warning text-uppercase">In Process</span></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>12408506525</td>
                            <td>Akihiro Leonel Juarez</td>
                            <td>KFC</td>
                            <td>₱840.00</td>
                            <td>09 Jul,2022 2:05 PM</td>
                            <td><span class="badge badge-soft-warning text-uppercase">In Process</span></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>49922144244</td>
                            <td>Ashianna Kim Fernandez</td>
                            <td>Chowking</td>
                            <td>₱148.00</td>
                            <td>09 Jul,2022 2:06 PM</td>
                            <td><span class="badge badge-soft-secondary text-uppercase">On the Way</span></td>
                            <td>09 Jul,2022 2:50 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>91103905867</td>
                            <td>Sebastian Vincent Camero</td>
                            <td>Mang Inasal</td>
                            <td>₱95.00</td>
                            <td>09 Jul,2022 2:10 PM</td>
                            <td><span class="badge badge-soft-warning text-uppercase">In Process</span></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>95121078447</td>
                            <td>Amora Elyse Ledezma</td>
                            <td>Max's Restaurant</td>
                            <td>₱689.00</td>
                            <td>09 Jul,2022 2:15 PM</td>
                            <td><span class="badge badge-soft-secondary text-uppercase">On the Way</span></td>
                            <td>09 Jul,2022 3:00 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>51036233833</td>
                            <td>Clyden Jaile Ramirez</td>
                            <td>Tropical Hut</td>
                            <td>₱171.00</td>
                            <td>09 Jul,2022 2:20 PM</td>
                            <td><span class="badge badge-soft-danger text-uppercase">Rejected</span></td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 2:30 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>31013186484</td>
                            <td>Samantha Maureen Vera</td>
                            <td>BonChon Chicken</td>
                            <td>₱215.00</td>
                            <td>09 Jul,2022 2:30 PM</td>
                            <td><span class="badge badge-soft-success text-uppercase">Delivered</span></td>
                            <td>09 Jul,2022 2:50 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 3:15 PM</td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>47604977923</td>
                            <td>Larkin Olivier Sanchez</td>
                            <td>Kenny Rogers Roasters</td>
                            <td>₱1,130.00</td>
                            <td>09 Jul,2022 2:48 PM</td>
                            <td><span class="badge badge-soft-success text-uppercase">Delivered</span></td>
                            <td>09 Jul,2022 3:20 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 4:00 PM</td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>38149334166</td>
                            <td>Avianna Rye Diaz</td>
                            <td>Mister Donut</td>
                            <td>₱350.00</td>
                            <td>09 Jul,2022 2:50 PM</td>
                            <td><span class="badge badge-soft-danger text-uppercase">Rejected</span></td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 3:00 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>17324343906</td>
                            <td>Ciandrei Kyle Lopez</td>
                            <td>Giligan's Restaurant</td>
                            <td>₱1,107.00</td>
                            <td>09 Jul,2022 2:58 PM</td>
                            <td><span class="badge badge-soft-secondary text-uppercase">On the Way</span></td>
                            <td>09 Jul,2022 3:25 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>16795163186</td>
                            <td>Kierra Zylene Ynares</td>
                            <td>Shakey's Pizza</td>
                            <td>₱305.00</td>
                            <td>09 Jul,2022 3:00 PM</td>
                            <td><span class="badge badge-soft-secondary text-uppercase">On the Way</span></td>
                            <td>09 Jul,2022 3:27 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>22836983523</td>
                            <td>Avrielle Haven Fernandez Juarez</td>
                            <td>David's Tea House</td>
                            <td>₱145.00</td>
                            <td>09 Jul,2022 3:14 PM</td>
                            <td><span class="badge badge-soft-danger text-uppercase">Rejected</span></td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 3:20 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                        <tr>
                            <td>78787993808</td>
                            <td>Estella Nataleigh Valeria Martinez</td>
                            <td>Classic Savory</td>
                            <td>₱305.00</td>
                            <td>09 Jul,2022 3:35 PM</td>
                            <td><span class="badge badge-soft-success text-uppercase">Delivered</span></td>
                            <td>09 Jul,2022 3:55 PM</td>
                            <td><span class="badge bg-danger text-uppercase">Not Applicable</span></td>
                            <td>09 Jul,2022 4:10 PM</td>
                            <td>
                                <button class="btn btn-sm btn-soft-info">View</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
    <!--end col-->
</div>
<!--end row-->